...
function [HS1, DC, H1] = HeartROI_2(I, A, B, C, D)
 ...
 for slice  = 1:C
     HS0 = zeros(A, B, 1, D);
     for time = 1:D
         % does Fourier transform frame by frame in 2D
        Magnitude = (fft2(I(:,:, slice, time)));
        % size(Magnitude) is 216 by 256
        % store frames for each slice on whole time sequence
        HS0(:,:,1,time) = Magnitude;
        % size(HS0)is 216 by 256 by 1 by 30
     end
    % does absolute inverse Fourier transform for each slice, 
    % giving us the average on first index, then harmonics linked to
    % movement
...
% There we extract average and first harmonic on slices
